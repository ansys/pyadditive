...
class AdditiveMachine:
    ...
    def __init__(self, **kwargs):
        # We don't use kwargs here in order to prevent a user from partially
        # defining an AdditiveMachine and later using an invalid instance
    ...
    @classmethod
    def from_machine_message(cls, msg: MachineMessage):
        if isinstance(msg, MachineMessage):
            ...
            raise ValueError(
                "Invalid type {} passed to {}.from_machine_message()".format(
                    type(msg), cls.__class__.__name__
                )
            )
